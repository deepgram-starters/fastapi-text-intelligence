...
FastAPI demo app for Deepgram Text Intelligence.
...
| File | Purpose |
|------|---------|
| `app.py` | Main backend — API endpoints and request handlers |
...
- **Backend:** `requirements.txt` — FastAPI with uvicorn. Uses Python venv for isolation.
...
```
feat(fastapi-text-intelligence): add diarization support
fix(fastapi-text-intelligence): resolve WebSocket close handling
refactor(fastapi-text-intelligence): simplify session endpoint
